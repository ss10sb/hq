...
useEcho(`game-access.${props.game.id}`, '.game.started', () => {
    gameReady.value = true;
});

const { channel } = useEchoPresence(`game.waiting-room.${props.game.id}`, [], () => {});
onMounted(() => {
                                ...
                                <div class="flex flex-col gap-2 border-b border-gray-200 px-2 py-1.5 dark:border-gray-700">
                                    <div class="flex flex-row items-center justify-between gap-2">
                                        <div class="flex items-center gap-2">
                                            ...
                                            <span v-if="p.isYou" class="rounded bg-primary/10 px-2 py-0.5 text-xs text-primary">You</span>
                                            <span
                                                v-if="p.isGM"
                                        ...
                                        <div v-if="isPresentInRoom(p)" class="flex items-center gap-2">
                                            <span class="inline-block h-3 w-3 rounded-full bg-green-500" aria-label="Present"></span>
                                        </div>
                                        <div v-else>
                                            <span class="inline-block h-3 w-3 rounded-full bg-gray-400" aria-label="Not present"></span>
                                        </div>
                                    </div>
                                    <div class="ms-5 mb-2 text-sm">
