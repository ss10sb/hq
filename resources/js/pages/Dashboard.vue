                ...
                <div class="p-3">
                    ...
                    <div v-if="games.data.length" class="@container">
                        <div v-for="game in games.data" :key="game.id"
                            ...
                            <Link v-if="game.status === GameStatus.InProgress" :href="PlayGameController(game.id)"
                                  :class="linkClasses"
                            >
                                {{ displayName(game) }}
                            </Link>
                            <Link v-else-if="game.status === GameStatus.Pending" :href="WaitingRoomController(game.id)"
                                  :class="linkClasses"
                            >{{ displayName(game) }}
                            </Link>
                            ...
                            <Form v-if="showDelete(game)"
                                  :action="DeleteGameController(game.id)"
