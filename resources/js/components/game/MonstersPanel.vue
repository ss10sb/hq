...
<template>
    <div class="mb-4">
        <div class="flex items-center justify-between gap-2 mb-1">
            <div class="text-xs uppercase text-gray-500">Monsters</div>
            <button v-if="selectedMonster" type="button" class="p-1 rounded hover:bg-gray-100 dark:hover:bg-neutral-800" @click="emit('close')">
                <X class="size-4" />
            </button>
        </div>
        ...
        <template v-else>
            <div class="p-2 border rounded-md border-gray-200 dark:border-neutral-800 bg-gray-50 dark:bg-neutral-900/50">
                <div class="flex items-center justify-between">
                    <div class="font-medium" :class="{ 'text-gray-500': selectedMonster.stats?.currentBodyPoints === 0 }">{{ selectedMonster.name }}</div>
                ...
                <div class="mt-2 flex flex-row items-center gap-5 text-gray-800 dark:text-white text-sm">
